...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "name",
            sa.Enum("USER", "ADMIN", "MODERATOR", name="usergroupenum"),
            nullable=False,
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("name"),
    ...
    op.create_index(
        op.f("ix_movies_certification_id"), "movies", ["certification_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "cart_id", "movie_id", name="uq_cart_items_cart_id_movie_id"
        ),
    )
    op.create_table(
        "order_items",
    ...
    op.create_index(
        "ix_payments_user_order", "payments", ["user_id", "order_id"], unique=False
    )
    op.create_table(
        "payment_items",
        sa.Column("id", sa.Integer(), nullable=False),
        ...
        sa.CheckConstraint(
            "price_at_payment >= 0", name="ck_payment_items_price_nonneg"
        ),
        sa.ForeignKeyConstraint(
